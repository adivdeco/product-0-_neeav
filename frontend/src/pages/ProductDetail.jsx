...
const ProductDetail = () => {
    ...
    // --- STATE ---
    const [product, setProduct] = useState(null);
    const [activeVariant, setActiveVariant] = useState(null); // NEW: Track selected variant
    const [similarProducts, setSimilarProducts] = useState([]);
    const [loading, setLoading] = useState(true);
    ...
    const isInCart = cartItems.some(item =>
        ...
        (!activeVariant || item.variantId === activeVariant._id)
    );
        ...
        const loadProduct = async () => {
            ...
            if (cachedProduct && isCacheValid) {
                ...
                if (cachedProduct.variants && cachedProduct.variants.length > 0) {
                    setActiveVariant(cachedProduct.variants[0]);
                }
                setLoading(false);
            } else {
                try {
                    ...
                    if (response.data.product.variants && response.data.product.variants.length > 0) {
                        setActiveVariant(response.data.product.variants[0]);
                    }
                } catch (error) {
                    toast.error('Product not found');
                } finally {
    ...
    const addToCart = async () => {
        ...
        if (product.variants?.length > 0 && !activeVariant) {
        ...
        try {
            await axiosClient.post('/cart/add', {
                productId: product._id,
                variantId: activeVariant?._id, // Pass variant ID
                quantity: quantity
            });
            toast.success(`Added ${activeVariant ? activeVariant.size || activeVariant.variantName : ''} to cart!`, { icon: '🛒' });
            dispatch(fetchCart());
        } catch (error) {
            console.error(error);
    ...
    const currentCostPrice = activeVariant ? activeVariant.costPrice : product?.costPrice;
    const currentStock = activeVariant ? activeVariant.stock : product?.stock;

    // Calculate discount dynamically
    const discount = currentCostPrice > currentPrice
        ? Math.round(((currentCostPrice - currentPrice) / currentCostPrice) * 100)
        : 0;
    ...
    return (
        <div className="min-h-screen bg-[#f1f3f6] font-sans pb-24 md:pb-10">
                ...
                {showReviewForm && (
                    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:px-4">
                        ...
                        >
                            ...
                            />
                            <button onClick={handleReviewSubmit} disabled={reviewLoading} className="w-full bg-blue-600 text-white py-3.5 rounded-lg font-bold">
                                {reviewLoading ? 'Submitting...' : 'Submit Review'}
                            </button>
                        </motion.div>
                    </div>
            ...
            <div className="max-w-7xl mx-auto md:pt-4">
                        ...
                        <div className="sticky top-20">
                            <div className="relative border border-gray-100 rounded-xl overflow-hidden bg-white aspect-[4/4] flex items-center justify-center group mb-4">
                                <motion.img
                                    ...
                                    className="w-full h-full object-contain p-4 transition-transform duration-500 group-hover:scale-105"
                                />
                                <button className="absolute top-4 right-4 p-2 bg-white/90 rounded-full shadow-md text-gray-400 hover:text-red-500 z-10"><Heart className="w-5 h-5" /></button>
                            </div>
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide justify-center">
                                {images.map((image, idx) => (
                                    <div
                                        ...
                                        onMouseEnter={() => setSelectedImage(idx)}
                                        onClick={() => setSelectedImage(idx)}
                                        className={`w-16 h-16 flex-shrink-0 border-2 rounded-lg p-1 cursor-pointer transition-all ${selectedImage === idx ? 'border-blue-600' : 'border-gray-200'}`}
                                    >
                                        <img src={image.url} alt="" className="w-full h-full object-contain" />
                                    </div>
                                ))}
                            ...
                            <div className="hidden lg:flex gap-4 mt-8">
                                ...
                                >
                                    {isInCart ? (
                                        <> <Check className='h-5 w-5 text-green-600' /> GO TO CART </>
                                    ) : (
                                        <> <ShoppingCart className="w-5 h-5 fill-white" /> {currentStock === 0 ? "OUT OF STOCK" : "ADD TO CART"} </>
                                    )}
                                </button>
                                <button
                                    onClick={buyNow}
                    ...
                    <div className="lg:col-span-7 p-4 md:p-6 lg:pl-10">
                        ...
                        <div className="mb-6">
                            <div className="flex items-baseline gap-3">
                                ...
                                {discount > 0 && (
                                    <>
                                        <span className="text-gray-500 text-lg line-through">₹{currentCostPrice?.toLocaleString()}</span>
                                        <span className="text-green-700 font-bold text-lg">{discount}% off</span>
                                    </>
                                )}
                            </div>
                            ...
                            {currentStock > 0 ? (
                                <p className="text-xs text-green-600 font-bold mt-1 flex items-center gap-1">
                                    In Stock
                                    {currentStock < 10 && <span className="text-red-500 font-normal ml-1">(Only {currentStock} left)</span>}
                                </p>
                            ) : (
                                <p className="text-xs text-red-600 font-bold mt-1">Currently Out of Stock</p>
                            )}
                        </div>
                        ...
                        {product.variants && product.variants.length > 0 && (
                            <div className="mb-8">
                                <div className="flex items-center gap-2 mb-3">
                                    <h4 className="text-sm font-bold text-gray-500 uppercase">Select Size / Variant</h4>
                                    <span className="text-xs text-blue-600 font-medium">
                                        {activeVariant ? `${activeVariant.size || ''} ${activeVariant.unit || ''}` : ''}
                                    </span>
                                </div>
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`
                                                    ...
                                                    min-w-[80px] flex flex-col items-center justify-center gap-1
                                                    ${isActive
                                                        ? 'border-blue-600 bg-blue-50 text-blue-700'
                                                        : isOutOfStock
                                                            ? 'border-gray-100 bg-gray-50 text-gray-300 cursor-not-allowed dashed-border'
                                                            : 'border-gray-200 text-gray-700 hover:border-blue-400'
                                                    }
                                                `}
                                            >
                                                <span>
                                                    {variant.variantName || `${variant.size} ${variant.unit}`}
                                                </span>
                                                {/* Optional: Show small price difference if needed */}
                                                {/* <span className="text-[10px] font-normal opacity-80">₹{variant.price}</span> */}

                                                {/* Out of Stock Strikethrough line (visual flair) */}
                                                {isOutOfStock && (
                        ...
                        {/* Description & Specifications (Same as before) */}
                        <div className="grid md:grid-cols-2 gap-8 mb-8">
                            <div>
                                <h4 className="text-sm font-bold text-gray-400 uppercase mb-4">Specifications</h4>
                                <div className="space-y-3 text-sm">
                                    <div className="flex"><span className="w-1/2 text-gray-500">Material</span><span className="w-1/2 font-medium">{product.material || 'Standard'}</span></div>
                                    <div className="flex"><span className="w-1/2 text-gray-500">Category</span><span className="w-1/2 font-medium">{product.category}</span></div>
                                    {activeVariant && (
                                        <>
                                            <div className="flex"><span className="w-1/2 text-gray-500">SKU</span><span className="w-1/2 font-medium">{activeVariant.sku || '-'}</span></div>
                                            <div className="flex"><span className="w-1/2 text-gray-500">Weight</span><span className="w-1/2 font-medium">{activeVariant.weightValue ? `${activeVariant.weightValue} ${activeVariant.unit}` : '-'}</span></div>
                                        </>
                                    )}
                                </div>
                            </div>
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">
                                                {product.shipping?.isFree ? 'Free Delivery' : `₹${product.shipping?.cost || 0} Shipping`}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="mb-8 bg-gray-100 rounded-2xl p-6 relative">
                            <h3 className="font-bold text-gray-900 mb-4 text-lg">Product Description</h3>
                            <div className={`relative text-sm text-gray-800 leading-relaxed prose prose-sm max-w-none transition-all duration-500 ease-in-out ${isDescExpanded ? 'max-h-full opacity-100' : 'max-h-[180px] overflow-hidden opacity-90'}`}>
                                <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(product.description || '') }} />
                            </div>
                            {!isDescExpanded && <div className="absolute blur-[5px] bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-gray-300 via-gray-100/90 to-transparent z-10 pointer-events-none" />}
                            <div className={`flex justify-center mt-4 ${!isDescExpanded ? 'absolute bottom-2 left-0 right-0 z-20' : ''}`}>
                                <button onClick={() => setIsDescExpanded(!isDescExpanded)} className="flex items-center gap-2 px-6 py-2 border border-gray-200 shadow-sm rounded-full text-sm font-bold text-white bg-black hover:shadow-md transition-all">
                                    {isDescExpanded ? 'Show Less' : 'Show More'} <ChevronRight className={`w-4 h-4 transition-transform ${isDescExpanded ? '-rotate-90' : 'rotate-90'}`} />
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="border-t border-gray-200 pt-8 mt-8">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold text-gray-900">Ratings & Reviews</h2>
                                {!userReview && <button onClick={() => setShowReviewForm(true)} className="shadow-sm bg-white border border-gray-300 px-4 py-2 rounded-lg font-medium text-sm hover:bg-gray-50">Rate Product</button>}
                            </div>

                            {/* Review List Logic (Same as before) */}
                            <div className="space-y-6">
                                {userReview && (
                                    <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 relative group">
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-xs">{user?.name?.charAt(0)}</div>
                                                <div>
                                                    <div className="flex items-center gap-2"><span className="font-bold text-sm text-gray-900">{user.name}</span><span className="bg-blue-100 text-blue-700 text-[10px] px-2 py-0.5 rounded-full font-bold">You</span></div>
                                                    <div className="flex items-center gap-2 mt-0.5"><div className="bg-green-700 text-white text-[10px] px-1.5 py-0.5 rounded flex items-center gap-0.5">{userReview.rating} <Star className="w-2.5 h-2.5 fill-white" /></div></div>
                                                </div>
                                            </div>
                                            <button onClick={handleReviewDelete} className="text-gray-400 hover:text-red-500 p-2"><Trash2 className="w-4 h-4" /></button>
                                        </div>
                                        <p className="text-gray-700 text-sm ml-11">{userReview.comment}</p>
                                    </div>
                                )}

                                {(() => {
                                    const otherReviews = product.rating?.reviews?.filter(r => r.userId?._id?.toString() !== user?._id?.toString()) || [];
                                    const reviewsToShow = isReviewsExpanded ? otherReviews : otherReviews.slice(0, 3);

                                    return (
                                        <div className="relative">
                                            {otherReviews.length === 0 && !userReview && <p className="text-gray-500 text-sm italic">No reviews yet.</p>}
                                            <div className="space-y-6">
                                                {reviewsToShow.map((review, i) => (
                                                    <div key={i} className="border-b border-gray-100 pb-6 last:border-0">
                                                        <div className="flex items-center gap-3 mb-3">
                                                            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-500">{review.userId?.name?.charAt(0)}</div>
                                                            <div>
                                                                <div className="font-bold text-sm text-gray-900">{review.userId?.name}</div>
                                                                <div className="bg-green-700 text-white text-[10px] px-1.5 py-0.5 rounded w-max flex items-center gap-0.5 mt-1">{review.rating} <Star className="w-2.5 h-2.5 fill-white" /></div>
                                                            </div>
                                                        </div>
                                            ...
                                            {!isReviewsExpanded && otherReviews.length > 3 && (
                                                <div className="flex justify-center mt-6">
                                                    <button onClick={() => setIsReviewsExpanded(true)} className="text-blue-600 text-sm font-bold flex items-center gap-1">See All Reviews <ChevronRight className="w-4 h-4 rotate-90" /></button>
                                                </div>
                                            )}
                                        </div>
                                    );
                ...
                {/* Similar Products (Same as before) */}
                {similarProducts.length > 0 && (
                    <div className="mt-4 bg-white p-4 md:p-6 shadow-sm md:rounded-lg">
                        <h2 className="text-lg font-bold text-gray-900 mb-4">Similar Products</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            {similarProducts.slice(0, 6).map((simProduct) => (
                                <Link to={`/product/${simProduct._id}`} key={simProduct._id} className="group">
                                    <div className="aspect-square bg-white border border-gray-200 rounded-lg overflow-hidden mb-2 relative">
                                        <img src={simProduct.ProductImage || simProduct.images?.[0]?.url} alt="" className="w-full h-full object-contain p-2 group-hover:scale-105 transition-transform" />
                                    </div>
                                    <h3 className="text-sm text-gray-600 truncate">{simProduct.name}</h3>
                                    {/* Display Lowest price for similar items */}
                                    <div className="font-bold text-gray-900">
                                        ₹{simProduct.variants && simProduct.variants.length > 0
                                            ? Math.min(...simProduct.variants.map(v => v.price)).toLocaleString()
            ...
            {/* Mobile Bottom Bar */}
            <div className="fixed text-center items-center bottom-0 left-0 right-0 bg-white shadow-lg z-50 flex lg:hidden h-16 px-4 py-2 gap-4 border-t border-gray-100">
                <button
                    onClick={handleCartAction}
                    disabled={currentStock === 0}
                    className={`flex-1 py-2 font-bold rounded-2xl text-lg transition-all uppercase tracking-wide flex items-center justify-center gap-2 ${isInCart
                        ? "bg-white text-black hover:bg-gray-200 border border-gray-300"
                ...
                <button onClick={buyNow} disabled={currentStock === 0} className="flex-1 py-2 bg-[#f96c26] text-white font-bold text-lg rounded-2xl shadow-md uppercase tracking-wide flex items-center justify-center gap-2">
                    <Zap className="w-5 h-5 fill-white" /> BUY NOW
                </button>
            </div>
