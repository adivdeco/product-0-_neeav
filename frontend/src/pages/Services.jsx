...
import { FiMapPin, FiSearch, FiFilter, FiX, FiArrowRight } from "react-icons/fi";
...
const ContractorSkeleton = () => (
    <div className="bg-white rounded-3xl p-4 animate-pulse border border-gray-100 shadow-sm">
        <div className="bg-gray-200 h-52 w-full rounded-2xl mb-4" />
        ...
        <div className="flex gap-3">
            ...
            <div className="h-12 bg-gray-200 rounded-2xl flex-[2]" />
        </div>
    </div>
);
...
function ContractorPage() {
    ...
    const [currentPage, setCurrentPage] = useState(1);
    const [showFilters, setShowFilters] = useState(false);
    const [filters, setFilters] = useState({ service: '', city: '', search: '' });
    ...
    const availableServices = [
        ...
        'Flooring', 'Roofing', 'Structural', 'Labor Supply', 'Construction',
        'Renovation', 'Demolition', 'Landscaping', 'HVAC', 'Welding'
    ];

    // API Fetch Logic
    const fetchContractors = useCallback(async (page = 1, currentFilters = filters) => {
        try {
            setLoading(true);
            const params = new URLSearchParams({
                page,
    ...
    useEffect(() => {
        if (user) fetchContractors(currentPage);
    }, [user, currentPage, fetchContractors]);
    ...
    const clearFilters = () => {
        const reset = { service: '', city: '', search: '' };
        setFilters(reset);
        setCurrentPage(1);
        fetchContractors(1, reset);
    };

    // Helper: Availability Badge Style
    const getAvailabilityBadge = (status) => {
        ...
        return (
            <span className={`${styles[status] || 'bg-gray-500'} px-3 py-1 rounded-full text-[10px] font-black tracking-widest text-white shadow-lg uppercase`}>
                {status?.replace('-', ' ')}
            </span>
        );
    };
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] pb-32">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-2 mb-8">
                    <div className="flex flex-col md:flex-row gap-2">
                        ...
                        <select
                            value={filters.service}
                            onChange={(e) => handleFilterChange('service', e.target.value)}
                            className="px-6 py-4 bg-transparent border-none focus:ring-0 text-sm font-semibold text-gray-700 cursor-pointer"
                        >
                            <option value="">All Services</option>
                        ...
                        <button
                            onClick={clearFilters}
                            className="bg-gray-900 text-white px-8 py-4 rounded-2xl hover:bg-blue-600 transition-all font-bold text-sm"
                        >
                            Reset
                        </button>
                    </div>
                ...
                {loading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {[1, 2, 3, 4, 5, 6].map(i => <ContractorSkeleton key={i} />)}
                    </div>
                ) : contractorData.contractors.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200">
                        <div className="text-6xl mb-4">🏗️</div>
                        <h2 className="text-2xl font-bold text-gray-800">No contractors found</h2>
                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {contractorData.contractors.map((contractor) => (
                                <div key={contractor._id} className="group bg-white rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 flex flex-col h-full">
                                    {/* Image Section */}
                                    <div className="relative h-64 overflow-hidden">
                                        ...
                                        {contractor.isVerified && (
                                            <div className="absolute top-5 right-5 bg-white/90 backdrop-blur-md p-2.5 rounded-2xl shadow-xl text-blue-600">
                                                <MdVerified size={22} />
                                            </div>
                                        )}
                                        <div className="absolute bottom-5 left-5">
                                            <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/20 text-white">
                                                <p className="text-[10px] uppercase font-bold opacity-70 tracking-tighter">Rate</p>
                                                <p className="text-sm font-bold">₹{contractor.pricing?.dailyRate || 'N/A'}/day</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-8 flex flex-col flex-grow">
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <h3 className="text-2xl font-black text-gray-900 tracking-tight mb-1">{contractor.contractorName}</h3>
                                                <div className="flex items-center text-gray-500 font-medium text-sm">
                                                    <FiMapPin className="mr-1 text-blue-500" /> {contractor.address?.city}
                                                </div>
                                            </div>
                                            <div className="bg-yellow-50 px-3 py-1.5 rounded-xl flex items-center shadow-sm">
                                                <MdStar className="text-yellow-500 mr-1" />
                                                <span className="font-black text-gray-800 text-sm">{contractor.rating?.average || '0'}</span>
                                            </div>
                                        </div>

                                        <p className="text-gray-500 text-sm leading-relaxed line-clamp-2 mb-6 italic">
                                            "{contractor.description || 'Professional contractor with a proven track record of quality delivery.'}"
                                        </p>

                                        <div className="flex flex-wrap gap-2 mb-8">
                                            {contractor.services?.slice(0, 3).map((s, i) => (
                                                <span key={i} className="px-3 py-1.5 bg-blue-50 text-blue-700 text-[11px] font-bold rounded-xl uppercase tracking-wider">
                                                    {s}
                                                </span>
                                            ))}
                                        </div>
                                        ...
                                        <div className="mt-auto flex gap-3">
                                            <Link
                                                to={`/contractor/${contractor._id}`}
                                                className="flex-1 text-center py-4 border-2 border-gray-100 rounded-2xl font-bold text-gray-600 hover:bg-gray-50 transition-all text-sm"
                                            >
                                                Profile
                                            </Link>
                                            <button
                                                onClick={() => { setSelectedContractor(contractor); setShowWorkRequestForm(true); }}
                                                className="flex-[2] bg-blue-600 text-white py-4 rounded-2xl font-bold text-sm shadow-lg shadow-blue-100 hover:bg-blue-700 flex items-center justify-center gap-2 group/btn transition-all"
                                            >
                                                Book Now
                        ...
                        {contractorData.totalPages > 1 && (
                            <div className="flex justify-center mt-16 gap-2">
                                {[...Array(contractorData.totalPages)].map((_, i) => (
                                    <button
                                        key={i + 1}
            ...
            {/* Modals & Layout Footer */}
            {showWorkRequestForm && selectedContractor && (
                <WorkRequestForm
                    contractorId={selectedContractor.contractorId || selectedContractor._id}
                    onSuccess={() => { setShowWorkRequestForm(false); alert('Request Sent!'); }}
